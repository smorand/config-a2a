...
def build_contents(messages: list[ChatMessage], codec: ToolNameCodec) -> tuple[str, list[dict[str, Any]]]:
    """Convert provider-agnostic ``ChatMessage`` history into Gemini ``contents`` (shared with Vertex AI)."""
...
def build_generate_content_payload(
    request: ChatRequest, codec: ToolNameCodec, system: str, contents: list[dict[str, Any]]
) -> dict[str, Any]:
    """Shape a Gemini ``:generateContent`` request body.

    Shared by the Google API-key and Vertex AI adapters: both endpoints speak
    the same wire format, only the transport (URL, auth) differs.
    """
...
def parse_generate_content_response(data: dict[str, Any], codec: ToolNameCodec) -> ChatResponse:
    """Parse a Gemini ``:generateContent`` response body (shared with Vertex AI)."""
